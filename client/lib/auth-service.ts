...
class AuthService {
  ...
  private verifyJWT(token: string): any {
    try {
      ...
      const expectedSignature = btoa(
        `${header}.${payload}.insygth_secret_2024`,
      );
  ...
  async ownerSignup(
    ...
    try {
      ...
      const ownerPasswordError = this.validateStrongPassword(
      ...
      const staffPasswordError = this.validateStrongPassword(
        signupData.staffPassword,
      );
  ...
  async staffSignin(
    ...
    try {
      ...
      if (
        signinData.businessName.trim().length < 2 ||
        signinData.businessName.trim().length > 100
      ) {
        return {
          success: false,
      ...
      if (
        signinData.name.trim().length < 2 ||
        signinData.name.trim().length > 50
      ) {
        return {
          success: false,
      ...
      if (
        signinData.staffPassword.length < 8 ||
        signinData.staffPassword.length > 50
      ) {
        return {
          success: false,
      ...
      if (!business) {
        return {
          success: false,
          message:
            "Business not found. Please check the business name or contact your business owner.",
        };
      }
      ...
      if (
        !this.verifyPassword(
          signinData.staffPassword,
          business.staffPasswordHash,
        )
      ) {
        return {
          success: false,
          message:
            "Invalid staff password. Please contact your business owner for the correct password.",
        };
      }
      ...
      for (const user of this.userDatabase.values()) {
        if (
          ...
          user.email.toLowerCase() === signinData.email.toLowerCase()
        ) {
          return {
            success: false,
            message:
      ...
      if (
        signinData.email.toLowerCase() === business.ownerEmail.toLowerCase()
      ) {
        return {
          success: false,
          message:
            "This email is reserved for the business owner. Staff members must use a different email address.",
        };
      }
      ...
      console.log(
        `Staff sign-in successful: ${user.name} (${user.role}) joined ${business.name}`,
      );
