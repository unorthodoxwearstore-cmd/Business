...
export function generateDashboardConfig(
  ...
  // Categorize widgets
  const primaryWidgets = widgets
    .filter((w) => w.category === "primary" && w.priority <= 4)
    .slice(0, 6);
  ...
  const analyticsWidgets = widgets
    .filter((w) => w.category === "analytics")
    .slice(0, 4);

  const settingsWidgets = widgets
    .filter((w) => w.category === "settings")
    .slice(0, 4);
  ...
  const kpis = KPI_CONFIGS.filter(
    (kpi) =>
      ...
      kpi.permissions.some((permission) =>
...
): "primary" | "secondary" | "analytics" | "settings" {
